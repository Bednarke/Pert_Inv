...
from mne.source_space import (_ensure_src, _filter_source_spaces,
                            _make_discrete_source_space, SourceSpaces)
from mne.source_estimate import VolSourceEstimate
from mne.surface import _normalize_vectors
from mne.bem import read_bem_solution, _bem_find_surface, ConductorModel
from mne.externals.six import string_types

from mne.forward import Forward, _merge_meg_eeg_fwds, convert_forward_solution
...
def _read_coil_def_file(fname, perts):
    ...
    if fname not in _coil_register:
        ...
        with open(fname, 'r') as fid:
            lines = fid.readlines()
        lines = lines[::-1] #This reverses the list
        while len(lines) > 0:
            line = lines.pop()
            if line[0] != '#':
